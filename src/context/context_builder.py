...
async def build_context_bundle(
    *,
    ...
    if session_runtime is not None:
        # 这一步先直接读取 SDK session 的现有历史，不做压缩、裁剪或 summary。
        history_items = list(await session_runtime.session.get_items())

    return ContextBundle(
        stable=build_stable_context_layer(tool_names),
        repo_rule=build_repo_rule_layer(),
        runtime=RuntimeContextLayer(
            history_items=history_items,
            current_turn_items=[{"role": "user", "content": user_input}],
        ),
    )
